	...
	for(int k = 0; k < n; k++){
		for(int i = 0; i < ELEM_PER_VECTOR; i++){
			...
			for(int j = 0; j < ELEM_PER_VECTOR; j++)
				pVecY[k][i] += pMatA[i][j] * pVecX[k][j];
		}
	}
}

__global__ void Mat_Vec_Multiply_Kernel(float (*X)[ELEM_PER_VECTOR], float (*Y)[ELEM_PER_VECTOR], float (*A)[ELEM_PER_VECTOR])
{
	int row = blockDim.y*blockIdx.y + threadIdx.y;
	int col = blockDim.x*blockIdx.x + threadIdx.x;
	int id = gridDim.x*blockDim.x*row + col;
...
void Mat_Vec_Multiply_GPU()
{
	...
	float (*d_X)[ELEM_PER_VECTOR], (*d_Y)[ELEM_PER_VECTOR], (*d_A)[ELEM_PER_VECTOR];
	size_t size = n * ELEM_PER_VECTOR * sizeof(float);
	size_t size_a = ELEM_PER_VECTOR * ELEM_PER_VECTOR * sizeof(float);
	...
	CUDA_CALL(cudaMemcpy(d_A, pMatA, size_a, cudaMemcpyHostToDevice))

	dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE);
	dim3 dimGrid(NX / dimBlock.x, NY / dimBlock.y);
	CHECK_TIME_START_GPU()
	Mat_Vec_Multiply_Kernel << < dimGrid, dimBlock >> > (d_X, d_Y, d_A);
	CHECK_TIME_END_GPU(device_time)
...
int main()
{
	...
	for (int i = 0 ; i < NX * NY ; i++){
		for (int j = 0 ; j < ELEM_PER_VECTOR ; j++){
			if (!(fabs(pVecY[i][j] - pVecY_G[i][j]) < 0.00001f)){
				printf("i : %d j : %d  not correct\n", i, j);
				return -1;
			}
		}
	}
