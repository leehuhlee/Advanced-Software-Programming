...
void CFilteringView::OnProblem3()
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for(k=-1*w;k<=w;k++){
				for(l=-1*w;l<=w;l++){
					if(r+k>=0&&r+k<m_height&&c+l>=0&&c+l<m_width){
						temp+=m_imageBits[(r+k)*m_width+c+l]*Sx[k+w][l+w];
						temp1+=m_imageBits[(r+k)*m_width+c+l]*Sy[k+w][l+w];
					}
				}
			}
			...
			gradient = temp*temp+temp1*temp1;
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for(k=-1*w;k<=w;k++){
				for(l=-1*w;l<=w;l++){
					if (r+k>=0&&r+k<m_height&&c+l>=0&&c+l<m_width){
						mean+=imageBits[(r+k)*m_width+c+l]*kernel[k+w][l+w];
					}
				}
			}
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			if(c==0){
				...
				l=c-w-1;
				for(k=-1*w;k<=w;k++){
					if(r+k>=0&&r+k<m_height&&l>=0&&l<m_width)
						mean-=m_imageBits[(r+k)*m_width+l]*kernel[k+w][0];
				}
				l=c+w;
				for(k=-w;k<=w;k++){
					if(r+k>=0&&r+k<m_height&&l>=0&&l<m_width)
						mean+=m_imageBits[(r+k)*m_width+l]*kernel[k+w][w0-1];
				}
			}
		...
		for (c=0; c<m_width; c++) {
			...
			for(k=-1*w;k<=w;k++){
				for(l=-1*w;l<=w;l++){
					...
					if(r+k>=0&&r+k<m_height&&c+l>=0&&c+l<m_width)
						medarray[(k+w)*w0+l+w]=m_imageBits[(r+k)*m_width+c+l];
					else
						medarray[(k+w)*w0+l+w]=0;
					}
				}
