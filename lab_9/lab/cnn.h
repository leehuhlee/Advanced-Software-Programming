				...
				for (int x = 0; x < size; x++) {
					for (int ky = 0; ky < conv->k_size; ky++) {														//이때 한 점은 kernel_size * kernel_size 번의 덧셈을 통해 계산
						for (int kx = 0; kx < conv->k_size; kx++) {
							...
							//
							// Convolution 과정입니다. 위에 만들어놓은 tmp 변수에 컨볼루션한 결과들을 저장하시면 됩니다.
							// Convolution의 정의를 생각하시고 index를 잘 고민해 보세요.
							//
							//
							////////////////////////////////////////////////////////////////////////////////
							/* ADD YOUR CODE HERE */
							tmp[y*size + x] += input2D[(y+ky) * input_size + x + kx] * kernel2D[ky*conv->k_size + kx];
						}
					}
				}
			}
