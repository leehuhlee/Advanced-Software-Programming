...
Mat make_2d_Gaussian_Kernel(int kernel_size, double sigma){

	Mat kx, ky, _2d_Gaussian_Kernel;
	
	/*	Needs to be implemented
		1. Define a 1D-gaussian kernel for each dimension(kx, ky)
		2. Do inner product between both kernels.
		3. Return 2D-Gaussian kernel(_2d_Gaussian kernel)
	*/

	kx = getGaussianKernel(kernel_size, sigma);
	ky = getGaussianKernel(kernel_size, sigma);
	_2d_Gaussian_Kernel = ky * kx.t();
	
	return _2d_Gaussian_Kernel;

}

//	YOON_ADDED
void my_cvSmooth(const IplImage* src, IplImage* dst, int kernel_size, double sigma1){
				
	Mat _2d_Gaussian_Kernel;					//	2D Gaussian Kernel to be implemented in the "make_2d_Gaussian_Kernel" function above
	int m_height = src->height;
	int m_width = src->width;	
	int channels = src->nChannels;					//	1 : grayscale image(current image), 3 : color image(RGB)
	int m_widthstep = src->widthStep;
	...
	w = floor((double)kernel_size/2);

	/*	Construct 2D Gaussian Kernel */
	_2d_Gaussian_Kernel = make_2d_Gaussian_Kernel(kernel_size, sigma1);

	//	Average Featuring
	for (r=0; r<m_height; r++){
		for (c=0; c<m_width; c++){
			/*	Needs to be implemented		
				1. Calculate the smoothing value by convoluting the original value(src) with 
					the pre-defined Gaussian kernel(_2d_Gaussian_Kernel).
				2. Restore its value into the new blurred image array(dst) for the next scale step
			*/
			
			gaussian=0;
			for(k=-w ; k<=w ; k++){
				for(l=-w ; l<=w; l++){
					if (r+k>=0&&r+k<m_height&&l+c>=0&&c+l<m_width){
						gaussian+=cvGet2D(src,r+k,c+l).val[0]*_2d_Gaussian_Kernel.at<double>(k+w,l+w);
					}
				}
			}
	...
	for( o = 0; o < octvs; o++ )
		for( i = 0; i < intvls + 3; i++ )
		{
			...
			else
			{
				...
				//my_cvSmooth(gauss_pyr[o][i-1], gauss_pyr[o][i], 0, sig[i]);		//	Yoon_Added(dynamically allocated kernel_size)
				my_cvSmooth(gauss_pyr[o][i-1], gauss_pyr[o][i], 9, sig[i]);		//	Yoon_Added(fixed kernel size)
			}
		}
...
ex2++;
								...
								if (debug==1)
									printf("    remove (intp), %d\n", nRemove++);
							}
						}

if (debug==1) {
	printf("\n%d extrema\n", ex1);
	printf("%d extrema, interpolate\n", ex2);
	printf("%d extrema, non-edge\n", features->total);
}
